...
from quant_platform.dashboard.components import apply_plotly_theme, render_section_banner
...
def render_correlation_view(aligned_close: pd.DataFrame):
    ...
    with h_col:
        ...
        fig_heat = go.Figure(data=go.Heatmap(
            ...
            text=text_vals,
            texttemplate="%{text}",
            textfont=dict(size=13, color="#ffffff", family="JetBrains Mono, monospace"),
            colorscale=[[0, "#f43f5e"], [0.5, "#18202c"], [1, "#3b82f6"]],
            zmin=-1.0,
            zmax=1.0,
    ...
    with r_col:
        ...
        fig_roll = go.Figure()
        pair_colors = ["#38bdf8", "#fb923c", "#a855f7", "#2dd4bf", "#f43f5e", "#10b981"]
        ...
        fig_roll.add_hline(y=0.0, line_dash="dash", line_color="rgba(255, 255, 255, 0.2)")
        ...
        for idx, (a1, a2) in enumerate(display_pairs):
            ...
            with insights_cols[idx % n_cols]:
                ...
                <div class="quant-card" style="border-top: 3px solid {border_c};">
                    ...
                    <div class="quant-card-value" style="color:{border_c};">{val:+.2f}</div>
